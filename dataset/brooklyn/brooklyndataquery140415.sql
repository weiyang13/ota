...
SET startdatetimes = CAST('2015-04-14 18:00:00 America/New_York' AS TIMESTAMP);
SET enddatetimes = CAST('2015-04-15 02:00:00 America/New_York' AS TIMESTAMP);
...
WITH table1 AS (
    ...
    WHERE trip_distance > 0.2
		...
		AND pickup_datetime >= startdatetimes
		AND pickup_datetime <= enddatetimes
  ), table2 AS (
...
SELECT times, 
	startx/avgvelocity AS startx, 
	starty/avgvelocity AS starty, 
	endx/avgvelocity AS endx, 
	endy/avgvelocity AS endy
FROM table2, `urop-ota2`.green.avvel
ORDER BY times